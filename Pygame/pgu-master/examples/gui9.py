...
class WelcomeDialog(gui.Dialog):
    def __init__(self,**params):
        ...
        doc.block(align=-1)
        doc.add(gui.Image("cuzco.png"),align=1)
        for word in """Wel3come to Cuzco's Paint.  Cuzco's Paint is a demo of the features of Phil's Pygame GUI.  Cuzco's Paint only supports saving in the .TGA format.""".split(" "): 
            doc.add(gui.Label(word))
            doc.space(space)
